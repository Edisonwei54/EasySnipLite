...
### 维护期：issue #23 缺陷修复（2026-08-13 本次；单测 203/203 全绿；verify-issue23 E2E PASSED；verify-issue20 回归 PASSED（含环境防抖后 7 连跑最终通过）——本机会话自 12:06 起间歇性异常：锁屏期间输入/截图失效、剪贴板写入间歇失败（PowerShell SetText 同样失败，系统级；用户侧应用正常）——两套 E2E 脚本均已加环境防抖（热键重试/剪贴板清空+轮询等待）应对；**PR #24 已合并进 main 6f2351d，issue #23 已关闭 COMPLETED，本地已收尾**）
...
- `tools/verify-issue20.ps1` — 环境防抖：Invoke-Capture 热键重试 + 场景前剪贴板清空（防陈旧内容误判）+ Save-Clipboard 轮询等待（复制与关窗解耦后读取需容忍短暂延迟/剪贴板竞争）+ UTF-8 BOM
...
6. **PS 5.1 读无 BOM 的 UTF-8 脚本按 ANSI 解析**：中文注释的 UTF-8 尾字节可能与换行组成 GBK 双字节对 → 注释吞掉后续整行导致解析错乱（本次 verify-issue20 中文字符串注释踩坑）——含中文的验证脚本必须保存为 UTF-8 带 BOM。
...
| #23 | 缺陷修复（实时标注预览 / 工具栏不被遮罩盖住 / 主体拖拽移动选区 / OnConfirm 先关窗再复制） | ✅ 已完成（单测 203/203；verify-issue23 + verify-issue20 E2E 均 PASSED，PR #24 已合并，issue #23 已关闭） |
