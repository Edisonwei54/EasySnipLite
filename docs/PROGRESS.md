...
### 维护期：issue #23 缺陷修复（2026-08-13 本次；单测 203/203 全绿；verify-issue23 E2E PASSED；verify-issue20 回归 PASSED（含环境防抖后 7 连跑最终通过）——本机会话自 12:06 起间歇性异常：锁屏期间输入/截图失效、剪贴板写入间歇失败（PowerShell SetText 同样失败，系统级；用户侧应用正常）——两套 E2E 脚本均已加环境防抖（热键重试/剪贴板清空+轮询等待）应对）
