...
> 当前进度：M0-M7 全部完成（热键唤起 → 冻结屏幕 → 框选 → 8 手柄调整 / 方向键微调 / 放大镜 → **内联标注**（issue #20，标注工具悬浮在选区下方，框选与标注同时进行，不再打开独立编辑器）→ 复制 / 保存 / 贴到屏幕）；滚动长截图已实现但决定跳过（代码存档，托盘入口已移除）；贴屏（M5）已完成；设置 + 三语（M6）已完成；打磨 + 发布（M7）已完成（版本 1.0.0）。单测 197/197。详见 [docs/PROGRESS.md](docs/PROGRESS.md)。
...
| 区域框选    | 拖拽框选，选区可 8 手柄调整、Alt+拖动移动、方向键微调；标注模式 Esc 三级退出（清标注→清选区→取消） | ✅ |
...
**使用**：启动后常驻托盘。任意应用按下 `Ctrl + 双击空格` → 拖拽框选区域 → 松开鼠标即进入**内联标注**：标注工具悬浮在选区下方（矩形/椭圆/箭头/画笔/荧光笔/马赛克/文字/表情，数字键 1-9 快速切换），颜色与线宽可直接选择，`Ctrl+Z`/`Ctrl+Y` 撤销重做，`Delete` 删除选中对象；框选仍可随时调整（8 手柄 / `Alt`+拖动移动选区 / 方向键微调），标注保留。`Ctrl+C` 复制 / `Ctrl+S` 保存 / 贴到屏幕，`Enter` 完成（复制并关闭），`Esc` 逐级清空标注 → 清空选区 → 退出。托盘右键菜单可再次唤起区域截图或退出。
