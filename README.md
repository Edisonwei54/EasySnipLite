...
> 当前进度：M0-M3 已完成（热键唤起 → 冻结屏幕 → 框选 → 8 手柄调整 / 方向键微调 / 放大镜 → **标注编辑器**：8 种工具 + 撤销重做 → 复制 / 保存 / 完成已可用）；滚动长截图已实现但决定跳过（代码存档，托盘入口已移除）；贴屏（M5）已完成，手工 E2E 已验证；设置 + 三语（M6）已完成（单测 176/176，手工 E2E 已验证）。详见 [docs/PROGRESS.md](docs/PROGRESS.md)。
...
| 托盘常驻    | 不占任务栏；开机自启可选（设置开启后写注册表 Run）     | ✅        |
