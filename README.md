...
> 当前进度：M0-M3 已完成（热键唤起 → 冻结屏幕 → 框选 → 8 手柄调整 / 方向键微调 / 放大镜 → **标注编辑器**：8 种工具 + 撤销重做 → 复制 / 保存 / 完成已可用）；滚动长截图已实现但决定跳过（代码存档，托盘入口已移除）；贴屏（M5）已完成，手工 E2E 已验证；设置 + 三语（M6）已完成（单测 167/167 全绿）。详见 [docs/PROGRESS.md](docs/PROGRESS.md)。

## ✨ 功能

| 功能        | 说明                                                   | 状态      |
| ----------- | ------------------------------------------------------ | --------- |
| 全局快捷键  | 任意应用按 **Ctrl + 双击空格** 唤起（设置窗口可自定义，含穿透热键） | ✅ |
...
| M6     | 设置页（快捷键录制/语言/保存目录/开机自启）+ 三语 | ✅ 完成（单测 167/167，手工 E2E 待终审） |
| M7     | 打磨 + 单文件发布                            | 待开发                  |
